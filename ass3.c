...
int isPermutation(int arr1[], int size1, int arr2[], int size2) {
   ...
   *******************************************************************************/
    ...
    //so it will work for every size of array
    int tableArr1[MAX]={0};
    int tableArr2[MAX]={0};

    //the size of table isn't important because we need only places for the values of arr 1/2
    createTable(arr1, tableArr1, size1);
    createTable(arr2, tableArr2, size2);
    for (int i = 0; i < MAX; i++) {
        //checking if the tables are equal if not then the arrays are not permutation
        if (tableArr1[i] != tableArr2[i])
            return 0;
    }
    //if we got here that means the tables are identical which means the original array are permutation
    return 1;
}
